# Gorjeio → Mensageiro em tempo real (estilo Telegram)

## Diagnóstico rápido do estado atual

Com base no código atual do Bird/ZIOS:

- A tela `Messages` já tem UI de conversa, contatos e input, mas depende de dados/funções (`users`, `messages`, `sendMessage`) que não existem no `BirdContext` atual.
- A tela `Notifications` também depende de `notifications` e `markNotificationRead`, igualmente ausentes no `BirdContext`.
...
**Conclusão:** o Gorjeio hoje é uma interface visual de chat, mas ainda não é um sistema de mensageria funcional em tempo real.
...
Evoluir o `BirdContext` para um estado de mensageria real:
...
1. `Messages.tsx` depende de campos/funções que não existem no `BirdContext` atual.
...
- `BirdContext` com store de mensagens real.
- Tela Gorjeio enviando e recebendo em tempo real.
...
Um "Gorjeio Telegram-like" pode ser considerado pronto quando:
